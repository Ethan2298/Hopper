...
This file provides guidance to Codex (and compatible agents) when working with code in this repository.
...
- `book-decorations.ts` — CodeMirror `StateField` that provides block `Decoration.replace` widgets over each unit's line range. Fold state is managed through `bookFoldEffect`.
...
The LLM contract is JSON-in, JSON-out. The system prompt in `main.js` (`describe-book-outline` handler) defines the `UnitProse` shape; the renderer supplies `{ index, kind, name, isMultiLine, source }` per unit and the LLM returns `{ prose: UnitProse[] }`. Line numbers and source ranges are owned by the renderer — the LLM never emits them.
